...
void MatchWorkThread::run() {
	try {
		...
		while (true) {
			...
			//单文件类型
				...
				while (true)
				{
					QByteArray bytes = file.read(WORK_READ_FILE_LENGTH);
					 match(bytes);
					//匹配到了或者文件读完了
					if (bytes.size() < WORK_READ_FILE_LENGTH) {
						break;
					}
				}
				file.close();
			...
			vector<MatcherResult*>* result = this->nowWork->matcher->check();
			if (result != NULL) {
